...
async function boot(page,{admin=false}={}){
    ...
    window.DTL_APP={
      ...
      async api(path,options={}){
        ...
        if(path==='/api/app/discovery/feed')return{
          fresh_novelpia:[{
            kind:'catalog',catalog_id:501,title:'Fresh RAW title',raw_available:true,
            raw_verification_status:'verified',
            raw_page_url:'https://raw-fucknovelpia.com/novel/401201',
            raw_verified_at:'2026-08-11T20:00:00Z',
          }],
        };
...
test('unverified or unavailable RAW never gets a verified badge',async({page})=>{
  ...
  await page.evaluate(async()=>{
    ...
    window.DTL_APP.api=async(path,options={})=>{
      if(path==='/api/app/discovery/feed')return{
        fresh_novelpia:[{
          kind:'catalog',catalog_id:501,title:'Unknown RAW title',raw_available:true,
          raw_verification_status:'error',raw_page_url:'https://raw-fucknovelpia.com/novel/401201',
        }],
      };
