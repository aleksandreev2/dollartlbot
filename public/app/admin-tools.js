(() => {
  ...
  async function renderUsers(){
    ...
    try{
      ...
      content(`<section class="admin-v3-users"><div class="admin-v3-toolbar admin-panel"><div class="admin-search">${icon('search')}<input id="adminUserSearch" value="${esc(userQuery)}" placeholder="@username, имя или Telegram ID"></div><div class="admin-v3-filters">${[['all','Все'],['boosty','Boosty'],['regular','Обычные'],['unlimited','Безлимит']].map(([id,label])=>`<button data-user-filter="${id}" class="${userFilter===id?'active':''}">${label}</button>`).join('')}</div><span class="admin-count">${Number(data.total||0)} пользователей</span></div><div class="admin-users-layout"><section class="admin-panel admin-users-list">${rows.length?rows.map(userRow).join(''):'<div class="admin-empty">Ничего не найдено.</div>'}</section><section class="admin-panel admin-user-detail" id="adminUserDetail"><div class="admin-user-placeholder">${icon('user-round-search')}<strong>Выберите пользователя</strong><span>Справа появится профиль, квота и история.</span></div></section></div></section>`);
