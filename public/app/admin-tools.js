(() => {
  ...
  if(!runtime?.registerPatcher)throw new Error('DTL runtime core must load before admin-tools.js');
  ...
  function content(html){const c=document.querySelector('.admin-content');if(c)c.innerHTML=html;icons();runtime.schedule();}
  ...
  async function renderUsers(){
    ...
    try{
      ...
      search?.addEventListener('keydown',e=>{if(e.key==='Enter'){clearTimeout(searchTimer);userQuery=e.currentTarget.value.trim();userOffset=0;renderUsers();}});
      search?.addEventListener('input',e=>{clearTimeout(searchTimer);const v=e.currentTarget.value.trim();searchTimer=setTimeout(()=>{if(v!==userQuery){userQuery=v;userOffset=0;renderUsers();}},420);});
      document.querySelectorAll('[data-user-filter]').forEach(b=>b.addEventListener('click',()=>{userFilter=b.dataset.userFilter;userOffset=0;renderUsers();}));
      document.getElementById('adminUserSort')?.addEventListener('change',e=>{userSort=e.currentTarget.value;userOffset=0;renderUsers();});
      document.querySelectorAll('[data-user-page]').forEach(b=>b.addEventListener('click',()=>{if(b.disabled)return;userOffset=Math.max(0,userOffset+(b.dataset.userPage==='next'?Number(data.limit||40):-Number(data.limit||40)));renderUsers();}));
      document.querySelectorAll('[data-user-id]').forEach(b=>b.addEventListener('click',()=>openUser(Number(b.dataset.userId))));
      if(selectedUser&&rows.some(x=>Number(x.telegram_id)===selectedUser))openUser(selectedUser);
  ...
  async function openUser(id){
    ...
    try{
      ...
      document.getElementById('refreshTelegramUser')?.addEventListener('click',()=>userAction(id,'refresh-telegram',{},'Профиль Telegram обновлён.'));
      document.getElementById('recheckUserAccess')?.addEventListener('click',()=>recheckUser(id));
      ...
      document.getElementById('saveUserControl')?.addEventListener('click',()=>saveUserControl(id));
      document.getElementById('toggleUserBlock')?.addEventListener('click',()=>toggleUserBlock(id,c));
      ...
      document.getElementById('toggleUnlimited')?.addEventListener('click',()=>changeQuota(id,{unlimited:!q.unlimited}));
      ...
      document.getElementById('sendAdminUserMessage')?.addEventListener('click',()=>sendUserMessage(id));
      icons();
  ...
  async function toggleUserBlock(id,c){
    ...
    busy=true;try{await api(`/api/app/admin/users/${id}/control`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});toast(blocked?'Пользователь разблокирован.':'Доступ пользователя заблокирован.');await openUser(id);await refreshUserRow();}catch(e){toast(e.message,true);}finally{busy=false;}
  }

  async function sendUserMessage(id){if(busy)return;const text=document.getElementById('adminUserMessageText')?.value.trim()||'';if(!text){toast('Введите сообщение.',true);return;}busy=true;try{await api(`/api/app/admin/users/${id}/message`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text})});toast('Сообщение отправлено.');await openUser(id);}catch(e){toast(e.message,true);}finally{busy=false;}}

  async function recheckUser(id){if(busy)return;busy=true;try{const d=await api(`/api/app/admin/users/${id}/recheck`,{method:'POST'}),member=d.channel?.member,sub=d.subscription?.subscriber;toast(`Канал: ${member===true?'участник':member===false?'не участник':'ошибка'} · Boosty: ${sub?'активен':'нет'}`,Boolean(d.channel?.error));await openUser(id);}catch(e){toast(e.message,true);}finally{busy=false;}}

  async function userAction(id,action,payload,success){if(busy)return;busy=true;try{await api(`/api/app/admin/users/${id}/${action}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload||{})});toast(success);await openUser(id);await refreshUserRow();}catch(e){toast(e.message,true);}finally{busy=false;}}
  async function refreshUserRow(){try{const qs=new URLSearchParams({filter:userFilter,sort:userSort,offset:String(userOffset)});if(userQuery)qs.set('q',userQuery);const d=await api(`/api/app/admin/users?${qs}`),body=document.querySelector('.admin-user-list-body');if(body)body.innerHTML=(d.users||[]).map(userRow).join('')||'<div class="admin-empty">Ничего не найдено.</div>';document.querySelectorAll('[data-user-id]').forEach(b=>b.addEventListener('click',()=>openUser(Number(b.dataset.userId))));document.querySelector(`[data-user-id="${selectedUser}"]`)?.classList.add('selected');icons();}catch{}}

  async function changeQuota(id,payload){if(busy)return;busy=true;try{const reason=document.getElementById('quotaReason')?.value.trim()||'';await api(`/api/app/admin/users/${id}/quota`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...payload,reason})});toast('Квота обновлена.');await openUser(id);await refreshUserRow();}catch(e){toast(e.message,true);}finally{busy=false;}}
  ...
  async function renderAnalytics(){
    ...
    try{const d=await api(`/api/app/admin/analytics?days=${analyticsDays}`),s=d.summary||{},max=Math.max(1,...(d.daily||[]).map(x=>Number(x.requests||0)));
      ...
      document.querySelectorAll('[data-days]').forEach(b=>b.addEventListener('click',()=>{analyticsDays=Number(b.dataset.days);renderAnalytics();}));
    }catch(e){error(e.message);}
  }
  ...
  async function renderPublications(){
    ...
    try{const d=await api('/api/app/admin/publishing'),rows=d.publications||[];content(`<section class="admin-publications-v3"><div class="admin-v3-toolbar admin-panel"><div><strong>${rows.length} публикаций</strong><span>Проверка комментариев и файлов выполняется автоматически через cron</span></div><button id="checkAllVisible">${icon('refresh-cw')} Проверить проблемные</button></div><div class="admin-publications-grid">${rows.length?rows.map(pubCard).join(''):'<div class="admin-empty admin-panel">Публикаций пока нет.</div>'}</div></section>`);
      document.querySelectorAll('[data-check-pub]').forEach(b=>b.addEventListener('click',()=>checkPublication(Number(b.dataset.checkPub),b)));
      document.getElementById('checkAllVisible')?.addEventListener('click',async()=>{for(const b of [...document.querySelectorAll('[data-check-pub]')].slice(0,10)){if(b.dataset.state!=='complete')await checkPublication(Number(b.dataset.checkPub),b,false);}toast('Проверка завершена.');});
    }catch(e){error(e.message);}
  }
  ...
  document.addEventListener('click',e=>{
    const custom=e.target.closest?.('[data-admin-tools]');if(custom){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();render(custom.dataset.adminTools);return;}
    const original=e.target.closest?.('[data-admin-section]');if(original){active='';selectedUser=null;userOffset=0;}
  },true);
