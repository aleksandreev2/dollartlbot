(() => {
  ...
  async function refresh(){if(!tg?.initData)return;try{const r=await fetch('/api/app/bootstrap',{headers:{'x-telegram-init-data':tg.initData},cache:'no-store'});if(!r.ok)return;const d=await r.json();account=d.account||null;locale=norm(d.user?.locale||document.documentElement.lang);patch();}catch{}}
  function patch(){if(!account?.unlimited)return;
    document.querySelectorAll('.premium-card').forEach(card=>{
      ...
      if(!card.querySelector('.quota-unlimited-stamp')){const s=document.createElement('div');s.className='quota-unlimited-stamp';s.textContent=`∞ ${words[locale]||words.en}`;card.appendChild(s);}
    });
