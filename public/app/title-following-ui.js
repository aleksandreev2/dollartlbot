(() => {
  ...
  async function convertDuplicateToDemand(original,duplicate){
    const submissionId=Number(duplicate?.submission_id);
    if(!Number.isSafeInteger(submissionId)||submissionId<=0)return false;
    try{
      const summary=await original('/api/app/discovery/interest',{
        method:'POST',headers:{'content-type':'application/json'},
        body:JSON.stringify({submission_id:submissionId,interested:true}),
      });
      return !summary?.own_request&&Boolean(summary?.viewer_interested);
    }catch{return false;}
  }

  function duplicateError(duplicate,converted){
    const error=new Error(converted?tx('duplicateConverted'):tx('duplicateOwn'));
    error.code='duplicate_title';
    error.submission_id=Number(duplicate?.submission_id)||null;
    error.quota_used=false;
    return error;
  }
    ...
    app.api=async(path,options={})=>{
      if(path==='/api/app/submit'&&options?.body instanceof FormData){
        ...
        if(identity.externalId||identity.sourceUrl){
          const preflight=await original('/api/app/submission/preflight',{
            method:'POST',headers:{'content-type':'application/json'},
            body:JSON.stringify({provider:identity.provider,external_id:identity.externalId,source_url:identity.sourceUrl}),
          });
          if(preflight?.duplicate?.submission_id){
            const converted=await convertDuplicateToDemand(original,preflight.duplicate);
            const existingId=Number(preflight.duplicate.submission_id);
            if(preflight.duplicate.queue_status&&Number.isSafeInteger(existingId)&&existingId>0){
              setTimeout(()=>app.openNovel?.(existingId),120);
            }
            throw duplicateError(preflight.duplicate,converted);
          }
        }
