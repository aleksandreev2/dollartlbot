(() => {
  ...
  if(!runtime?.registerPatcher)throw new Error('DTL runtime core must load before admin-publishing.js');
  ...
  async function installManagement(){
    ...
    const d=await managementData().catch(()=>null);if(!d)return;
  ...
  document.addEventListener('click',e=>{if(e.target.closest?.('[data-admin-tools="publications"]')){managementCache=null;runtime.schedule();}},true);
  ...
  runtime.registerPatcher(()=>{install();void installManagement();});
})();
