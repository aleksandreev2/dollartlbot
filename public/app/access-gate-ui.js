(() => {
  ...
  const ACCESS_CODES = new Set(['membership_required', 'access_check_unavailable', 'access_restricted']);
  ...
  function renderLocked(payload) {
    ...
    const title = details.title || (restricted
      ? 'Dollar TL access is unavailable'
      : unavailable
        ? 'Access check is temporarily unavailable'
        : 'Join the Dollar TL channel');
    const message = error.message || (restricted
      ? 'Access to Dollar TL is currently unavailable for this account.'
    ...
    app.viewRoot.innerHTML = `<section class="page access-gate-page"><div class="surface-card access-gate-card">
      <div class="access-gate-icon">${icon(restricted ? 'shield-x' : unavailable ? 'refresh-cw' : 'radio-tower')}</div>
      <h1>${app.escapeHtml(title)}</h1>
      <p>${app.escapeHtml(message)}</p>
      <div class="access-gate-actions">
        ${!restricted && joinUrl ? `<button class="primary-button wide-button" id="accessGateJoin" type="button">${icon('send')} ${app.escapeHtml(joinLabel)}</button>` : ''}
  ...
  app.init = async function accessAwareInit() {
    ...
    try {
      // Opening the Mini App must reflect current membership and admin restrictions,
      // not a previous positive cache entry from before the user's state changed.
      const result = await requestAccess(true);
