(() => {
  ...
  const {state,viewRoot,tr,copy,escapeHtml,languageFlag,languageName,cover,relativeTime}=app;

  function renderHome() {
    const b=state.bootstrap, a=b.account, active=b.queue.active?.[0], mine=b.my_requests||[];
    const name=escapeHtml(b.user.first_name || b.user.username || copy('reader'));
    viewRoot.innerHTML=`<section class="page stagger">
      ...
      <section class="section"><div class="section-header"><h2>${escapeHtml(tr('myRequests'))}</h2><button class="link-button" id="homeRequests" type="button">${escapeHtml(tr('viewAll'))} ›</button></div>${mine.length?`<div class="surface-card simple-list">${mine.slice(0,3).map(requestListRow).join('')}</div>`:emptyCard('▤',copy('noRequests'))}</section>
    </section>`;
