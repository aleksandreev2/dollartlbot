...
export async function deliverPublicationPayload(
  ...
  if(publication.add_bot_comment===1&&publication.bot_comment_status!=='sent'){
    ...
    const donation=(await setting(env,'donation_url'));
    ...
    try{
      ...
      await env.DB.prepare(`UPDATE publications SET bot_comment_status='sent',bot_comment_message_id=?,bot_comment_error=NULL WHERE id=?`).bind(sent.message_id,publicationId).run();
      await log(env,publicationId,'success','bot_comment_sent','Рекламный комментарий Dollar TL Bot подтверждён.');
    }catch(error){
      const message=friendly(error);
      await env.DB.prepare(`UPDATE publications SET bot_comment_status='failed',bot_comment_error=? WHERE id=?`).bind(message.slice(0,1000),publicationId).run();
      await log(env,publicationId,'error','bot_comment_failed','Не удалось отправить рекламный комментарий.',String(error));
    }
  }else if(publication.add_bot_comment===0){
    await env.DB.prepare(`UPDATE publications SET bot_comment_status='disabled' WHERE id=?`).bind(publicationId).run();
  }

  await finalize(publicationId,env);
}
...
async function checkPublicationDelivery(id:number,env:Env,telegram:TelegramClient,manual:boolean){
  ...
  const assetSummary=await env.DB.prepare(`SELECT COUNT(*) total,SUM(CASE WHEN delivery_status='sent' THEN 1 ELSE 0 END) sent,SUM(CASE WHEN delivery_status='failed' THEN 1 ELSE 0 END) failed FROM publication_assets WHERE publication_id=?`).bind(id).first<Record<string,number>>();
...
async function finalize(id:number,env:Env){
...
async function linkedDiscussionId(env:Env,telegram:TelegramClient):Promise<number|null>{
  const channel=await setting(env,'publish_channel_id');if(!channel)return null;
