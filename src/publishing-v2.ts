...
function normalizedHeading(value:string){return value.trim().replace(/\s+/g,' ').toLocaleLowerCase();}
function bodyStartsWithTitle(body:string,title:string){const firstLine=body.split(/\r?\n/,1)[0]||'';return Boolean(firstLine.trim()&&normalizedHeading(firstLine)===normalizedHeading(title));}
function composePost(p:PublicationRow,s:Settings){const body=p.body_html.trim(),title=p.internal_title.trim();const parts=[];if(title&&!bodyStartsWithTitle(body,title))parts.push(`<b>${escapeHtml(title)}</b>`);if(body)parts.push(escapeHtml(body));if(p.add_footer){const u=`https://t.me/${s.bot_username||'dollartlbot'}?start=submit`;parts.push(`<b>Need a translation?</b>\nOpen <a href="${escapeHtml(u)}">Dollar TL Bot</a> and suggest a novel for translation.`);}return parts.filter(Boolean).join('\n\n');}
