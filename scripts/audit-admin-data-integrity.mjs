...
const need = (source, token, label = token) => { if (!source.includes(token)) throw new Error(`Missing ${label}`); };
const forbid = (source, token, label = token) => { if (source.includes(token)) throw new Error(`Forbidden ${label}`); };
...
for (const token of [
  "'/api/app/admin/list'",
  "type AdminListKind = 'pending' | 'active' | 'completed' | 'rejected' | 'all' | 'queue'",
  'ADMIN_LIST_LIMIT_DEFAULT = 30',
  'ADMIN_LIST_LIMIT_MAX = 50',
...
console.log('Admin data integrity audit passed: atomic queue reorder/normalization and server-side request filters/search/cursor pagination are wired through canonical runtimes.');
