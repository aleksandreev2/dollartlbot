...
function need(source, token, label = token) { if (!source.includes(token)) throw new Error(`Admin console routes: missing ${label}`); }
function forbid(source, token, label = token) { if (source.includes(token)) throw new Error(`Admin console routes: forbidden ${label}`); }
...
for (const token of [
  ...
  "document.dispatchEvent(new CustomEvent('dtl:adminrender'",
]) need(consoleJs, token);

for (const token of [
  'renderPublishing', 'renderBroadcasts', 'bindPublisher', 'createPublication', 'pubTitle', 'pubBody',
  "['overview','publishing','broadcasts','settings']", 'scheduleLegacyAdminMigration', '.admin-stats', 'sessionStorage', 'button.click()',
]) forbid(consoleJs, token);

for (const token of [
  "admin.registerRoute('section:publishing'", 'publisher-editor', 'pubTitle', 'pubBody', 'pubPublish', '/api/app/admin/publishing', 'bindPreview', 'bindPublicationRows', 'unmount: clearPreviewUrl',
