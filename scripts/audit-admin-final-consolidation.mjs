...
const need = (source, token, label = token) => {
  if (!source.includes(token)) throw new Error(`Admin final consolidation: missing ${label}`);
};
const forbid = (source, token, label = token) => {
  if (source.includes(token)) throw new Error(`Admin final consolidation: forbidden ${label}`);
};
...
for (const source of [runtime, stability, consoleJs]) {
  forbid(source, 'dtl:admin:last-section', 'legacy admin storage key');
}
for (const token of ['restoreNav', 'selectorForToken', 'saveNav', 'activeNavButton']) forbid(stability, token, `stability ${token}`);
for (const token of ['renderPublishing', 'renderBroadcasts', 'scheduleLegacyAdminMigration', '.admin-stats', 'dtl:adminrender']) forbid(consoleJs, token, `console ${token}`);
...
for (const token of [
  ...
  "window.DTL_ADMIN.activeRoute()",
  "section:publishing",
  "tools:publications",
  "section:broadcasts",
  "tools:users",
  "health:1",
  'admin integration mutation',
  'autosave survives real module navigation',
  'mobile admin shell does not overflow',
...
console.log('Admin final consolidation audit passed: no legacy route storage/replay, split canonical route owners, one Publishing Center navigation surface, and full integration browser coverage.');
