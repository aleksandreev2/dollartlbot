...
const sharedModules={
  ...
  'publishing-fixes':'public/app/publishing-fixes.js',
  'publication-management':'public/app/publication-management.js',
};
...
if(!adminPerf.includes('runtime.registerFetchMiddleware'))throw new Error('admin-performance-v3 must register cache middleware with shared fetch pipeline.');
if(/window\.fetch\s*=/.test(adminPerf))throw new Error('admin-performance-v3 must not wrap fetch directly.');

for(const legacy of ['/app/i18n-wizard.js','/app/i18n-complete.js','/app/ui-polish.js','/app/publishing-comments-ui.js'])if(index.includes(legacy))throw new Error(`Legacy runtime must not be loaded: ${legacy}`);
for(const removed of ['public/app/i18n-wizard.js','public/app/i18n-complete.js','public/app/ui-polish.js','public/app/publishing-comments-ui.js'])if(fs.existsSync(new URL(`../${removed}`,import.meta.url)))throw new Error(`Legacy runtime must be removed: ${removed}`);

const order=['/app/i18n-core.js','/app/i18n-runtime-v2.js','/app/novel-presenter.js','/app/admin-performance-v3.js','/app/publishing-fixes.js','/app/publication-management.js','/app/app.js'];
let previous=-1;
...
console.log('Shared Mini App runtime audit passed: one DOM observer, one fetch wrapper, shared middleware, and shared enhancement scheduling.');
